...
class CounriesViewController: UIViewController {
    ...
    var countryBtclosure : (( Int?,String) -> Void)? = nil
...
extension CounriesViewController: UITableViewDataSource, UITableViewDelegate{
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        self.dismiss(animated: false, completion: { [self] in
            self.countryBtclosure!(counties[indexPath.row].id, MOLHLanguage.currentAppleLanguage() == "en" ? (counties[indexPath.row].nameEn ?? "") : (counties[indexPath.row].nameAr ?? ""))
        })
    }
}
